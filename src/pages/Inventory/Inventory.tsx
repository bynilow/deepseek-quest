...
const Inventory = () => {
    ...
    const itemsList = useMemo(() => {
        ...
        if (foundedGame?.messages) {
            console.log('FOUNDED')
            const assistantMessagesWithItems =
                foundedGame?.messages
            ...
            const messagesWithItems =
                assistantMessagesWithItems.map(
                    message => message?.includes(DELETED_ITEM_SEPARATOR) || message?.includes(RECEIVED_ITEM_SEPARATOR)
            ...
            const replacedItems =
                messagesWithItems.map(message => message?.split(ACTIONS_GROUP_SEPARATOR)[0].replace(';', '').replace('\n', '').trim());
